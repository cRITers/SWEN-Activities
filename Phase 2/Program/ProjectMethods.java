...
public class ProjectMethods implements EventHandler<ActionEvent> {
   ...
   ArrayList<TextField> foodsTextFields = new ArrayList<TextField>();
   ...
   public void doViewCatalog() {
      ...
      foodTextArea.appendText(fileManager.getCatalogue());
   ...
   public void doAddFood() {
      foodsTextFields = new ArrayList<TextField>();
      backCheck = true;
      addCheck = true;
      ...
      foodsTextFields.add(tfCalories);
      ...
      foodsTextFields.add(tfProtein);
      fpProtein.getChildren().addAll(lblProtein, tfProtein);
   ...
   public void addFoodLog() {
      ...
      try {
         Food food = new Food(foodsTextFields.get(0).getText(), Double.parseDouble(foodsTextFields.get(1).getText()),
               Double.parseDouble(foodsTextFields.get(2).getText()),
               Double.parseDouble(foodsTextFields.get(3).getText()),
               Double.parseDouble(foodsTextFields.get(4).getText()));
         foodsTextFields.get(0).setText("");
         foodsTextFields.get(1).setText("");
         foodsTextFields.get(2).setText("");
         foodsTextFields.get(3).setText("");
         foodsTextFields.get(4).setText("");
         fileManager.addItem(food);
         System.out.println("Successfully added the food");

      } catch (Exception e) {
         System.out.println("Error doing addFoodLog, error msg:" + e.getMessage());
      }
   ...
   public void doAddRecipe() {
      ...
      Button btnBack = new Button("Back");
      Button btnAdd = new Button("Add New Food to Recipe");
      btnBack.setOnAction(this);

      Button btnRemoveFood = new Button("Remove Newly added");
      btnRemoveFood.setVisible(false);

      Button btnAddFull = new Button("Add Recipe to Catalogue");
      ...
      btnAddFull.setOnAction(new EventHandler<ActionEvent>() {
         public void handle(ActionEvent evt) {
            try {
               /**
                * Recipe recipe = new Recipe(); recipe.setName(tfName.getText());
                * recipe.addItem(new ItemAdapter(item, count)); fileManager.addItem(recipe);
                * System.out.println("Successfully added the food");
                */
      ...
      botPane.setPadding(new Insets(10, 0, 0, 0));
      botPane.setHgap(30);
      botPane.getChildren().addAll(btnBack, btnRemoveFood, btnAdd, btnAddFull);
